...
```
    ...
    ## Warning in inner_join(world_data, combined_happiness, by = "country"): Detected an unexpected many-to-many relationship
    ## between `x` and `y`.
...
```
    ...
    ## Warning in inner_join(world_data, hp_avg, by = "country"): Detected an unexpected many-to-many relationship
    ## between `x` and `y`.
    ## ℹ Row 11 of `x` matches multiple rows in `y`.
